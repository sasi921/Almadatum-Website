...
<html>
 ...
 <body>
  <div class="container box">
   <h1 align="center">HIGHER STUDIES INFORMATION</h1>
   <br />
...
<script type="text/javascript" language="javascript" >
 $(document).ready(function(){
  ...
  $(document).on('click', '#insert', function(){
   var SID = $('#data1').value();
   var admission_year = $('#data2').value();
   var university = $('#data3').value();
   var continent = $('#data4').value();

   if(SID != '' && admission_year != ''  && university != '' && continent != '')
   {
    $.ajax({
     url:"insert2.php",
     method:"POST",
     data:{SID:SID,admission_year:admission_year,university:university,continent:continent},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
